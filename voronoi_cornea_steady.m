...
function [layer,resultTAC,resultTDC,resultLESC,resultex] = voronoi_cornea_steady(layersun)
...
resultTAC = zeros(tfinal/dt,2);
resultTDC = zeros(tfinal/dt,2);
resultLESC = zeros(tfinal/dt,2);
resultex = zeros(tfinal/dt,2);
...
X_basal = layersun(1).cells(:,1);
Y_basal = layersun(1).cells(:,2);
...
celltypes_basal(1:N_rim) = layersun(1).cells(1:N_rim,3);
celltypes_basal((N_rim+1):end) = layersun(1).cells((N_rim+1):end,3);
% % Set ID for each LESC
ID_basal  = zeros(length(X_basal),1);
ID_basal(1:N_rim) = layersun(1).cells(1:N_rim,4);    % All LESCs get an ID 1, 2, 3, ...
ID_basal ((N_rim+1):end) = layersun(1).cells((N_rim+1):end,4);
% % Set numbers of division to 0
ND_basal = zeros(length(X_basal),1);
ND_basal(1:N_rim) = layersun(1).cells(1:N_rim,5);    % All LESCs get an ID 1, 2, 3, ...
ND_basal ((N_rim+1):end) = layersun(1).cells((N_rim+1):end,5);

clock_basal = zeros(length(X_basal),1);
clock_basal(1:N_rim) = layersun(1).cells(1:N_rim,6);    % All LESCs get an ID 1, 2, 3, ...
clock_basal ((N_rim+1):end) = layersun(1).cells((N_rim+1):end,6);
...
for ii=2:n_layer
    ...
    X_layern = layersun(2).cells(:,1);
    Y_layern = layersun(2).cells(:,2);
    n = 0;
    ...
    celltypes_layern(1:N_rim) = layersun(2).cells(1:N_rim,3);
    celltypes_layern((N_rim+1):end) = layersun(2).cells((N_rim+1):end,3);

    ID_layern = zeros(length(X_layern),1);
    ID_layern(1:N_rim) = layersun(2).cells(1:N_rim,4);    % All LESCs get an ID 1, 2, 3, ...
    ID_layern ((N_rim+1):end) = layersun(2).cells((N_rim+1):end,4);

    ND_layern = zeros(length(X_layern),1);
    ND_layern(1:N_rim) = layersun(2).cells(1:N_rim,5);    % All LESCs get an ID 1, 2, 3, ...
    ND_layern((N_rim+1):end) = layersun(2).cells((N_rim+1):end,5);

    clock_layern = zeros(length(X_layern),1);
    clock_layern(1:N_rim) = layersun(2).cells(1:N_rim,6);    % All LESCs get an ID 1, 2, 3, ...
    clock_layern((N_rim+1):end) = layersun(2).cells((N_rim+1):end,6);
...
for t = dt:dt:tfinal
     ...
     number_of_cells = size(layer(s).cells, 1); 
        for i_cell = 1:number_of_cells
          ...
          if layer(s).cells(i_cell,i_type)==v_TAC    
                ...
                if (layer(s).cells(i_cell,i_nd) == n_max) && (rand < 0)   %0.008 %0.00001
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if s==1
           nT = sum(layer(s).cells(:,i_type) == v_divideTAC);
           nex =  sum(layer(s).cells(:,i_type) == v_extruded);
 % 
           dTAC = sum((layer(s).cells(:,i_nd)  < n_max)&(layer(s).cells(:,i_type) == v_TAC));
           dTDC =  sum(layer(s).cells(:,i_type) == v_TAC);
 % % 
           fractionTAC = nT / dTAC;
           fractionTDC = nT / dTDC;
 ...
 % 
 % % % 
           resultTAC(t/dt,1) = fractionTAC;
           resultTAC(t/dt,2) = t;
 % % 
 % % 
           resultTDC(t/dt,1) = fractionTDC;
           resultTDC(t/dt,2) = t;

           resultex(t/dt,1) = fractionex;
           resultex(t/dt,2) = t;

       end
       if s==1
           nL = sum(layer(s).cells(:,i_type) == v_divideLESC);

           fractionLESC = nL /50;
           
           resultLESC(t/dt,1) = fractionLESC;
           resultLESC(t/dt,2) = t;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Finding
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% tip
% % % % Let's say we want to consider the basal layer
%     s0 = 2;
%     % Let's check the tip corresponding to id = 2
%     id0 = 4;
% 
%     indx = (layer(s0).cells(:, i_id) == id0); % Indices of TACs that have id = id0
% 
%     % Vector of distances from centre of all cells in layer s0 with id = id0
%      distance_TAC = sqrt(layer(s0).cells(indx, i_xcoord).^2 + layer(s0).cells(indx, i_ycoord).^2);
%     % 
%     % Distance of tip from centre
%     if ~isempty(distance_TAC)
%         matrix_tip(m) =  min(distance_TAC);
%     else
%         matrix_tip(m) = R;
%     end
...
for s=1:n_layer
    ...
    % daspect([1, 1, 1]);
         subplot(1,2,s)
  plotter_steady(plotfigure,layer(s).cells,i_xcoord,i_ycoord,R,i_id,i_type,v_LESC,t);
end
end
